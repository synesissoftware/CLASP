...
* Added **`clasp::arguments_adaptor_t`** so **`showUsage()`** / **`showVersion()`** accept **`arguments_t`** by pointer or by reference;
* Added **`clasp::main::invoke()`** overload whose callback takes **`arguments_t const&`**;
